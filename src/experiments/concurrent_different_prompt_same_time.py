...
MAX_TOKENS = 1
BASE_DIR = r"C:\Users\ASUS\Desktop\Pars\HardwareAware\src\experiments\data\rendering\concurrent_different_prompt_same_time"
    ...
    try:
        with concurrent.futures.ThreadPoolExecutor(max_workers=client_count) as executor:
            ...
            for i in range(client_count):
                prompt.append(generate_prompt(prompt_length, seed=i))
            for i in range(client_count):
                future = executor.submit(send_request, i, csv_path, prompt[i])
                futures[future] = i
